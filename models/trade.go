...
// AttachAlgoOrderRequest represents a single attached TP/SL element sent in
// PlaceOrderRequest.AttachAlgoOrds via the OKX attachAlgoOrds mechanism.
//
// Unlike AttachAlgoOrder (a response DTO with non-optional string fields),
// all fields here are pointers so that unset fields are omitted from the
// JSON payload rather than serialized as empty strings.
//
// Set TpOrdPx/SlOrdPx to "-1" to execute the TP/SL as a market order upon
// trigger. TpTriggerPxType/SlTriggerPxType accept "last", "index", or "mark".
type AttachAlgoOrderRequest struct {
	AttachAlgoClOrdID *string `json:"attachAlgoClOrdId,omitempty"`
